...
import { themeStore, itemStore } from '../store/StoreList';
// ItemList 컴포넌트
export const ItemList = memo(() => {
  ...
  useMemo(() => {
    itemStore.setState((prev) => ({
      items: [...prev.items, ...generateItems(1000, prev.items.length)],
    }));
  }, []);

  const { items, addItems } = itemStore.useStore();
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <div>
          <button
            ...
            onClick={() => addItems(generateItems(1000, items.length))}
          >
            대량추가
          </button>
        </div>
